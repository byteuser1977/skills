...
| `--recursive` | bool | `true` | 递归子目录 |
| `--keep-structure` | bool | `true` | 保持目录结构 |
...
| 文件 | 说明 |
|------|------|
| `scripts/batch_convert_direct_md.py` | 优化版批量转换（按目录分组）|
| `scripts/batch_convert_doc_to_md_final.py` | 最终版（三步分类处理）|
| `scripts/scan_directory.py` | 仅扫描统计文件类型 |
| `scripts/rebuild_knowledge_index.py` | 重建知识库索引 |
